...
class ListChecker(Checker):
    ...
    def check_data(self, r: Redis, cross_slots_cmd: bool):
        for i in range(self.cnt):
            ...
            p.execute_command("EXHGET",f"{self.PREFIX}_{self.cnt}", "field")
            p.execute_command("EXHGET",f"{self.PREFIX}_{self.cnt}_ABS", "field_abs")
            p.execute_command("EXHGET",f"{self.PREFIX}_{self.cnt}_EX", "field_ex")

            p.execute_command("EXHGET",f"{self.PREFIX}_{self.cnt}_ALL_01", "field_all_01")
            p.execute_command("EXHGET",f"{self.PREFIX}_{self.cnt}_ALL_01", "field_all_02")

            p.execute_command("EXHGET",f"{self.PREFIX}_{self.cnt}_ALL_02", "field_all_01")
            p.execute_command("EXHGET",f"{self.PREFIX}_{self.cnt}_ALL_02", "field_all_02")
